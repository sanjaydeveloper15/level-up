...
class WebSocketController extends Controller implements MessageComponentInterface
{
    ...
    public function onOpen(ConnectionInterface $conn)
    {
        ...
        foreach ($this->clients as $client) {
            $client->send(json_encode([
                "type" => "total_online",
                "total_online" => count($this->userresources),
                "message" => "Total Online: " . count($this->userresources),
                "subscribe" => count($this->subscriptions),
                "userresources" => count($this->userresources)
            ]));
        }
    }
    /**
    ...
    public function onClose(ConnectionInterface $conn)
    {
        ...
        foreach ($this->clients as $client) {
            $client->send(json_encode([
                "type" => "total_online",
                "total_online" => count($this->clients),
                "message" => "Total Online: " . count($this->clients)
            ]));
        }
    }
